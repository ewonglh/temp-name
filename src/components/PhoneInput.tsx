...
const randomizeDialLayout = () => {
    const numbers: (number | string)[] = [1, 2, 3, 4, 5, 6, 7, 8, 9, 0, '+', '*'];
...
export default function PhoneInput({ 
    ...
    const [dialLayout, setDialLayout] = useState<(number | string)[]>(() => randomizeDialLayout());
    ...
    useEffect(() => {
        if (dialVisible && dialContainerRef.current) {
            ...
            document.documentElement.style.paddingBottom = `${dialHeight}px`;
    ...
    const getNumberAtPointer = useCallback((currentRotation: number): number | string | null => {
        ...
        const slotIndex = Math.round(adjustedAngle / slotAngle) % dialLayout.length;
    ...
    const handleDialMouseDown = (e: React.MouseEvent) => {
        ...
        if (dialRef.current) {
            ...
            const angle = Math.atan2(startY - centerY, startX - centerX) * (180 / Math.PI);
            ...
            const slotIndex = Math.round(clickAngleOnDial / slotAngle) % dialLayout.length;
    ...
    const handleTouchMove = useCallback((e: TouchEvent) => {
        if (!isDragging || !dialRef.current || !dragStartRef.current || e.touches.length === 0) return;
        ...
        <Box className="phone-input-container">
            ...
            {dialVisible && (
                <div 
                    ...
                    className="rotary-dial-container"
                >
                    <div className="rotary-dial-wrapper">
                        ...
                        >
                            {dialLayout.map((num, index) => {
                                const angle = (index * 360) / dialLayout.length; // Start at 0 degrees (right side)
                                const radius = 100;
                                const x = Math.cos((angle * Math.PI) / 180) * radius;
                                const y = Math.sin((angle * Math.PI) / 180) * radius;
                                
                                const pointerNumber = getNumberAtPointer(rotation);
                                const isAtPointer = pointerNumber === num && isDragging && pointerNumber === draggedNumberRef.current;
